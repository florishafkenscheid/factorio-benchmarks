...
import { BenchmarkResult, MetricValue } from "../data/BenchmarkResult"
...
export const createLineChartForMetrics = (result: BenchmarkResult, options: LineChartOptions): ChartConfiguration<"bar" | "line"> => {
    ...
    result.metrics.filter(it => supportedMetrics[it.name] !== undefined).forEach(metric => {
        ...
        const data = result.metricValues.get(metric.name).filter(it => it.tick <= options.maxTicks).map(it => ({ x: it.tick, y: nanoToMicro(it.value) }));
        // sort by tick ascending
        data.sort((a, b) => a.x - b.x);
        datasets.push({
            label: result.fileName + " - " + metric.name,
            data: data,
    ...
    const wholeUpdateAverage = nanoToMicro(metricValueAverage(result.metricValues.get(MetricEnum.WHOLE_UPDATE.name)))

    datasets.push({
        type: "line",
        label: result.fileName + " - Whole Update Averge",
